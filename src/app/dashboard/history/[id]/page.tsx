...
import { getMachineStatus } from "@/lib/runner";
...
export default async function MachineDetail({
  ...
  const machineId = (await params).id;
              ...
              <p className="text-sm">
                {machine.createdAt
                  ? `${format(
                      ...
                      "PPpp"
                    )} (${formatDistanceToNow(new Date(machine.createdAt), {
                      addSuffix: true,
                    })})`
                  : "Unknown"}
              </p>
        ...
        <CardContent>
          {events.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No events found for this machine.
            </p>
          ) : (
